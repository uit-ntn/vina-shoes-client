...
export default function WishlistPage() {
  ...
  useEffect(() => {
    const fetchWishlist = async () => {
      try {
        setLoading(true);
        // Get wishlist from API with populated product details
        const wishlistData = await wishlistService.getWishlist(true);
  ...
  const handleAddToCart = async (productId: string) => {
    try {
      ...
      if (wishlistItem && wishlistItem.product) {
        ...
        const size = product.sizes && product.sizes.length > 0 ? product.sizes[0] : 39;
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-10">
          ...
          <h4 className="mt-3 text-lg font-medium text-gray-800">Vui lòng đăng nhập</h4>
          <p className="mt-1 text-gray-600">Đăng nhập để xem danh sách yêu thích của bạn.</p>
          <Link href="/login?redirect=/profile/wishlist">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {wishlistItems.map((item) => {
            // Skip if product data isn't available
                ...
                <div className="p-4">
                  ...
                  <div className="mt-2 flex items-center">
                    ...
                    <span className="ml-1 text-xs text-gray-600">
                      {/* Real rating would come from reviews */}
